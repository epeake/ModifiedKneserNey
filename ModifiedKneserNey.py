...
class Smooth:
    ...
    def log_score_per_ngram(self, corpus):
        """
       Given a corpus outside of training data.  Finds the average ngram log probability of the corpus.              NEED TO ACTUAL CALC THIS LOL

       :param corpus: String.  ASCII encoded corpus to score
       :return: average ngram log probability
       """
        probability_keys = self._padded_email_ngrams(corpus, self.highest_order)

        for i in range(0, len(probability_keys)):
            if (probability_keys[i][-1],) not in self._get_vocab():
        ...
        for i in range(0, len(scentence_probabilities)):
            if scentence_probabilities[i] is None:  # this is the case for completely unknown
                scentence_probabilities[i] = log(self.av_unk)
            else:
